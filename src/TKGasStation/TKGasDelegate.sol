...
/// @dev Implements EIP-712 for typed structured data signing, supporting multiple execution modes including standard execution, batch execution, and sessions
...
contract TKGasDelegate is EIP712, IERC1155Receiver, IERC721Receiver, IERC1271, ITKGasDelegate {
    ...
    function _executeNoValueNoReturn(
        ...
        assembly {
            ...
            if iszero(call(gas(), _outputContract, 0, ptr, _arguments.length, 0, 0)) { mstore(0x00, EXECUTION_FAILED_SELECTOR) revert(0x00, 0x04) }
    ...
    function _executeWithValueNoReturn(
        ...
        assembly {
            let ptr := mload(0x40)
            calldatacopy(ptr, _arguments.offset, _arguments.length)
            if iszero(call(gas(), _outputContract, _ethAmount, ptr, _arguments.length, 0, 0)) { mstore(0x00, EXECUTION_FAILED_SELECTOR) revert(0x00, 0x04) }
    ...
    function _executeBatchSessionNoReturn(
        ...
        for (uint256 i = 0; i < length; ++i) {
            ...
            assembly ("memory-safe") {
                ...
                if iszero(call(gas(), outputContract, ethAmount, ptr, _callData.length, 0, 0)) { mstore(0x00, EXECUTION_FAILED_SELECTOR) revert(0x00, 0x04) }
    ...
    function _executeBatchNoReturn(
        ...
        for (uint256 i = 0; i < length; ++i) {
            ...
            assembly ("memory-safe") {
                ...
                if iszero(call(gas(), outputContract, ethAmount, ptr, _callData2.length, 0, 0)) { mstore(0x00, EXECUTION_FAILED_SELECTOR) revert(0x00, 0x04) }
